...
* execute device [[Computer Science/Architecture/Function|Subroutine]], e.g. mouse interrupt
...
If a device isn't interrupting, it passes through the INT signal, else it asserts its device ID onto the Data bus, so that the [[Control Unit]] can save it to the ETR. Then that value in the ETR is used as an index into the IVT so that the [[Program Counter|PC]] can be set to the beginning of the interrupt [[Computer Science/Architecture/Function|Subroutine]].
## RETI
...
* An *unscripted [[Computer Science/Architecture/Function|Subroutine]] call*, triggered by an external event
